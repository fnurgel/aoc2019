...
int calculate_output(array(int) phases, array(int) int_codes) {
    array fifos = ({});
    ...
    do {
        ...
        thread_create(execute, int_codes - ({}), input_fifo, output_fifo);
    ...
    execute(int_codes - ({}), input_fifo, first_fifo);
    return first_fifo.read();
}
...
int main() {
    ...
    foreach(file->line_iterator(true); int idx; string line) {
        ...
        foreach(indices(find_all_unique_combinations(55555, 99999, (<5,6,7,8,9>))), int phases) {
            array phase_str = (sprintf("%05d", phases) / "");
            phase_str = map(phase_str, lambda(string v) { return (int)v; });
            int output = calculate_output(phase_str, int_codes - ({}));
